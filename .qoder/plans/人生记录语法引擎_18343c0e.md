...
```
用户输入 → debounce 500ms → 调用 /api/v2/parse（LLM 解析）
    |
    v
[语义卡片区]（替代当前的芯片区）
    ├─ 主体行：[主语] [动作] [宾语]
...
- **输入**：`{ text: string, current_date: string, items?: {id,title}[] }`
- **输出**：`ParsedResult`
- **逻辑**：
  1. 规则预处理（提取金额/时长/指标）
...
- **输入**：`{ record_id: string, search_keyword: string, date_hint: string }`
- **输出**：候选关联记录列表
- **逻辑**：按 date_hint 范围 + keyword 模糊搜索历史记录
