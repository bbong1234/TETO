...
subgraph "数据库层"
H[004_record_type_convergence.sql<br/>类型收敛]
...
| 类型 | 颜色编码 | 用途 | 关键特征 |
|------|----------|------|----------|
| 发生 | 绿色 | 已完成的行为记录 | 实际发生的事件，有具体的时间戳 |
| 计划 | 蓝色 | 待执行的任务计划 | 未来将要执行的活动，支持生命周期管理 |
| 想法 | 橙色 | 思维记录和灵感 | 创造性思维产物，可作为后续行动的起点 |
| 总结 | 灰色 | 反思和学习成果 | 对过往经验的整理和提炼 |
...
graph TB
subgraph "表现层"
UI[React组件<br/>RecordItem, FilterBar]
Client[RecordsClient<br/>状态管理]
end
subgraph "业务逻辑层"
API[API路由<br/>/api/v2/records]
Service[业务服务<br/>记录操作]
end
subgraph "数据访问层"
DB[数据库操作<br/>Supabase ORM]
Cache[查询缓存<br/>性能优化]
end
subgraph "数据存储层"
Records[records表<br/>核心数据]
