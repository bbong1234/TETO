...
- 当前用户工具：支持开发模式与生产模式切换；在开发模式下返回固定用户 ID；在生产模式下通过 Supabase 客户端获取当前登录用户，未登录时抛出错误。**双端保护**：服务端（`src/lib/auth/server/get-current-user-id.ts`）和客户端（`src/lib/auth/get-current-user-id.ts`）各自独立实现生产环境自动禁用逻辑，确保 DEV_MODE 不会在生产环境激活。
...
- 开发模式：通过环境变量开启开发模式，绕过真实认证，便于本地调试与演示。**生产环境保护**：服务端（`DEV_MODE`）和客户端（`NEXT_PUBLIC_DEV_MODE`）均实现了生产环境自动禁用——当 `NODE_ENV=production` 时，DEV_MODE 会被强制设为 `false` 并输出 `console.error` 警告，而非抛出异常（避免阻断 `next build`）。这确保即使 `.env` 中残留 DEV_MODE=true，生产环境也不会绕过 RLS 或跳过认证。

章节来源
- [src/lib/auth/server/get-current-user-id.ts:5-12](file://src/lib/auth/server/get-current-user-id.ts#L5-L12)
- [src/lib/auth/get-current-user-id.ts:6-16](file://src/lib/auth/get-current-user-id.ts#L6-L16)
- [src/lib/supabase/server.ts:5-14](file://src/lib/supabase/server.ts#L5-L14)
...
- **DEV_MODE 生产泄漏**：若生产环境意外启用了 DEV_MODE，系统会自动禁用并输出 console.error 警告。检查 `.env` 和 `.env.local` 是否残留 `DEV_MODE=true` 或 `NEXT_PUBLIC_DEV_MODE=true`，确保生产部署前已移除。
...
## 附录
- 环境变量与开发模式：通过环境变量控制开发模式与开发用户 ID，便于本地调试。**安全机制**：服务端使用 `DEV_MODE`（非 NEXT_PUBLIC_），客户端使用 `NEXT_PUBLIC_DEV_MODE`；两者均在 `NODE_ENV=production` 时自动禁用（`DEV_MODE = false`），不会 throw 阻断构建，而是通过 `console.error` 输出警告。`src/lib/supabase/server.ts` 同样实现了此保护，确保 SERVICE_ROLE_KEY 不会在生产环境被使用。
