...
- 版本依赖
  ...
  - 1.5 新增 user_rules 表支持智能分类与路由，新增周期规则与数据性质字段支持规律识别与推断分析；**新增性能复合索引迁移 007**，以 `user_id` 为前缀的复合索引替代旧弱索引，并清理被覆盖的冗余索引。
- 外键与级联
...
- 索引策略
  ...
  - **RLS 友好复合索引（007 迁移）**：因为 RLS 策略基于 `auth.uid() = user_id` 过滤，所有索引必须以 `user_id` 为前缀才能被 PostgreSQL 查询计划器利用。007 迁移新增：`idx_records_user_item(user_id, item_id)`、`idx_records_user_phase(user_id, phase_id)`、`idx_records_user_sub_item(user_id, sub_item_id)`、`idx_records_user_time_anchor(user_id, time_anchor_date)`、`idx_goals_user_phase(user_id, phase_id)`、`idx_goals_user_sub_item(user_id, sub_item_id)`、`idx_items_user_folder(user_id, folder_id)`。
  - **部分索引（WHERE ... IS NOT NULL）**：对可空外键列建立部分索引，避免索引中包含大量 NULL 行。如 `idx_records_user_item WHERE item_id IS NOT NULL`。
  - **冗余弱索引清理**：007 迁移同时 DROP 了被复合索引覆盖的旧单列索引（如 `idx_records_item`、`idx_records_phase`、`idx_goals_item` 等），减少写入开销。
  - 按外键建立单列索引（旧策略，部分已被 007 复合索引替代）：records(item_id)、records(chain_id)、records(phase_id)、records(sub_item_id)、records(linked_record_id)、record_tags(record_id)、record_tags(tag_id)、items(user_id,status)、chains(user_id,item_id)、item_folders(user_id)、items(folder_id)、sub_items(user_id)、sub_items(item_id)。
...
- TETO 1.5：新增 user_rules 表支持智能分类与路由；新增周期规则与数据性质字段支持规律识别与推断分析；007 迁移新增 RLS 友好复合索引（以 user_id 为前缀），清理被覆盖的旧弱索引。
