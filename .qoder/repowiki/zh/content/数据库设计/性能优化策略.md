...
- [page.tsx（事项详情）](file://src/app/(dashboard)/items/[id]/page.tsx)
...
## 更新摘要
**变更内容**
- 新增并行计算和批量数据获取的最佳实践章节
- 更新Promise.all并行执行模式的实现细节
- 增加Map结构内存聚合优化的具体案例
- 扩展性能优化策略以涵盖现代异步编程模式
...
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [并行计算与批量数据获取最佳实践](#并行计算与批量数据获取最佳实践)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)
...
**更新** 新增并行计算和批量数据获取的最佳实践，包括Promise.all并行执行模式和Map结构内存聚合优化。
...
### Promise.all并行执行模式
TETO项目广泛采用了Promise.all并行执行模式来优化API响应时间和数据库查询性能。这种模式在多个关键场景中得到应用：

#### API路由层并行执行
在事项详情页面的API路由中，使用Promise.all并行计算阶段聚合数据：
```typescript
...
### Map结构内存聚合优化
项目中大量使用Map结构进行内存级别的数据聚合，显著提升了性能：

#### 前端组件中的Map聚合
在事项详情页面的子项级聚合计算中，使用Map结构进行内存聚合：
```typescript
const subItemAgg = useMemo(() => {
  ...
  return {
    ...
    metric_summaries: Array.from(metricMap.entries()).map(([name, { total, unit }]) => ({ 
      metric_name: name, 
      total_value: total, 
      metric_unit: unit 
    })),
...
#### 并行度控制
- **适度并行**：根据系统资源和数据库连接池大小，合理控制并行任务数量
- **错误处理**：使用Promise.allSettled或适当的错误处理机制确保单个任务失败不影响整体性能
- **超时控制**：为长时间运行的任务设置合理的超时机制

#### 内存优化技巧
- **增量聚合**：使用Map的get/set模式避免重复查找
- **类型安全**：为Map键值对定义明确的类型，提升TypeScript类型检查准确性
- **内存回收**：及时清理不需要的Map引用，避免内存泄漏

#### 性能监控
- **执行时间统计**：为并行任务添加执行时间监控
- **并发度分析**：监控同时执行的任务数量和数据库连接使用情况
- **资源使用跟踪**：监控内存使用和CPU占用情况

**章节来源**
- [route.ts（事项聚合）: 36-47:36-47](file://src/app/api\v2\items\{id}\route.ts#L36-L47)
- [route.ts（目标引擎）: 34-37:34-37](file://src/app/api\v2\items\{id}\goal-engine\route.ts#L34-L37)
- [insights.ts: 418-427:418-427](file://src/lib/db/insights.ts#L418-L427)
- [page.tsx（事项详情）: 284-304:284-304](file://src/app/(dashboard)/items/[id]/page.tsx#L284-L304)
- [insights.ts: 598-609:598-609](file://src/lib/db/insights.ts#L598-L609)
- [insights.ts: 700-710:700-710](file://src/lib/db/insights.ts#L700-L710)
- [insights.ts: 822-836:822-836](file://src/lib/db/insights.ts#L822-L836)
...
**更新** 新增的并行计算和批量数据获取最佳实践为系统性能优化提供了现代化的解决方案，Promise.all并行执行模式和Map结构内存聚合优化显著提升了API响应速度和内存使用效率。这些技术的应用需要在保证数据一致性的同时，合理控制并发度和内存使用，确保系统的稳定性和可扩展性。
