...
- [ParsedChip.tsx](file://src/app/(dashboard)/records/components/QuickInput/ParsedChip.tsx)
...
- [parse-rules-fallback.ts](file://src/lib/ai/parse-rules-fallback.ts)
...
## 项目结构
- 记录管理位于 dashboard/records，包含客户端页面 RecordsClient 与若干 UI 组件：RecordList、RecordItem、QuickInput（含子组件 ParsedChip）、RecordEditDrawer。
...
- 类型定义集中在 src/types/teto.ts；自然语言解析逻辑在 src/lib/utils/parseNaturalInput.ts；语义解析类型在 src/types/semantic.ts；AI推理和规则回退系统在 src/lib/ai/ 目录下。
...
subgraph "语义解析与AI推理"
SEM["semantic.ts"]
PF["parse-rules-fallback.ts"]
ER["enhance-record.ts"]
...
**图表来源**
- [QuickInput.tsx](file://src/app/(dashboard)/records/components/QuickInput.tsx#L1-L100)
- [semantic.ts:1-126](file://src/types/semantic.ts#L1-L126)
- [parse-rules-fallback.ts:1-422](file://src/lib/ai/parse-rules-fallback.ts#L1-L422)
- [enhance-record.ts:1-400](file://src/lib/ai/enhance-record.ts#L1-L400)
- [RecordEditDrawer.tsx](file://src/app/(dashboard)/records/components/RecordEditDrawer.tsx#L650-L702)
...
- 记录管理
  ...
  - QuickInput：快速输入与解析，支持本地即时解析、AI 增强、复合句拆分与双向语义互补，**新增语义卡片界面、AI推理显示、置信度级别、建议修正等功能**。**组件拆分**：ParsedChip 已提取为独立组件（`QuickInput/ParsedChip.tsx`），导出 `ChipData` 接口和 `ParsedChip` 默认组件，支持芯片点击编辑和删除。
  - ParsedChip：语义芯片组件，从 QuickInput 中提取的自包含组件。接收 `ChipData`（key/label/value/icon），支持点击进入编辑模式、回车确认、Esc 取消、X 删除。
...
- 数据流
  ...
  - **新增语义解析引擎**：QuickInput 通过 semantic.ts 类型定义支持完整的语义解析，包括置信度级别、风险等级、模糊输入分类等。
  - **新增规则回退系统**：当 AI 解析失败时，自动切换到本地规则解析模式，确保系统可用性。
...
participant AI as "AI解析引擎"
participant Fallback as "规则回退系统"
U->>Q : 输入自然语言
Q->>Q : 本地解析 parseNaturalInput
Q->>API : POST /records 创建记录
API-->>Q : 返回记录
Q->>Q : 触发AI增强
Q->>AI : POST /parse AI解析
AI-->>Q : 返回解析结果含置信度、推理
alt AI解析失败
Q->>Fallback : 触发规则回退
Fallback-->>Q : 返回本地解析结果
...
**图表来源**
- [QuickInput.tsx](file://src/app/(dashboard)/records/components/QuickInput.tsx#L529-L909)
- [RecordsClient.tsx](file://src/app/(dashboard)/records/RecordsClient.tsx#L231-L325)
- [parse-rules-fallback.ts:284-370](file://src/lib/ai/parse-rules-fallback.ts#L284-L370)
...
- 职责
  ...
  - **新增语义卡片界面**：通过 chips 数组构建语义卡片，展示解析出的字段。
  - **新增AI推理显示**：显示AI的判断理由和置信度级别。
  - **新增建议修正功能**：检测歧义并提供澄清选项。
  - **新增规则回退系统**：AI失败时自动切换到本地规则解析。
- 关键属性
  - selectedDate: string
  - tags: Tag[]
  - items: Item[]
...
- 关键行为
  ...
  - **新增语义卡片**：chips 数组构建语义卡片，支持点击跳转到事项。
  - **新增AI推理显示**：显示 parsed_semantic.reasoning 字段。
  - **新增置信度级别**：支持 field_confidence 和 confidence 字段。
  - **新增建议修正**：检测歧义并弹出澄清框，支持多种风险级别的处理。
  - **新增规则回退**：AI失败时自动切换到本地规则解析模式。
...
- 状态与数据绑定
  - 本地状态：rawText、parsed、type、content、selectedItemId、selectedTagIds、expanded、submitting、splitMode、splitIndex、splitNotice、clarification、fallbackMessage。
...
Enhance --> AIResult{"AI解析结果"}
AIResult --> |成功| MergeResult["合并解析结果"]
AIResult --> |失败| Fallback["触发规则回退"]
Fallback --> LocalParse["本地规则解析"]
LocalParse --> MergeResult
MergeResult --> Split{"复合句?"}
...
- 关键行为
  ...
  - **新增AI推理显示**：显示 parsed_semantic.reasoning 字段和低置信度提示。
- 状态与数据绑定
...
SEM["semantic.ts<br/>语义解析类型"] --> QI["QuickInput.tsx"]
SEM --> RED["RecordEditDrawer.tsx"]
PF["parse-rules-fallback.ts<br/>规则回退系统"] --> QI
ER["enhance-record.ts<br/>增强记录逻辑"] --> QI
PNI["parseNaturalInput.ts<br/>本地解析"] --> QI
QI --> RC
...
- **新增语义解析优化**
  - QuickInput 通过 chips 数组缓存语义卡片，避免重复计算。
  - 澄清框采用30秒超时机制，防止长时间占用UI。
...
- AI 增强异常
  ...
  - **新增规则回退**：当 AI 解析失败时，系统会自动切换到本地规则解析模式。
- 批量删除
  - RecordsClient 的批量删除在确认后调用 /api/v2/records/batch-delete，失败时显示错误并保持选择状态。
- 洞察数据加载
  - InsightsClient 在请求失败时显示错误与"重新加载"按钮，检查日期范围与网络状态。
- **新增语义解析问题**
  - QuickInput 的语义卡片显示异常：检查 chips 数组构建逻辑和字段映射。
  - AI推理显示不正确：检查 parsed_semantic.reasoning 字段是否存在。
  - 置信度提示不准确：检查 field_confidence 和 confidence 字段的计算逻辑。
...
## 结论
TETO 的核心业务组件围绕"记录—洞察—项目跟踪"形成闭环：记录管理强调"即时保存 + 后台增强"的体验与多维语义解析；洞察统计提供可视化的数据概览；项目跟踪通过目标与阶段卡片、时间线与文件夹提升长期项目的可追踪性。**新增的语义卡片界面、AI推理显示、置信度级别、建议修正功能以及规则回退系统**使得系统在保持高性能的同时，提供了更智能、更可靠的自然语言处理能力。组件间通过明确的属性与回调解耦，结合统一的类型定义与状态管理模式，既满足业务理解，又便于技术实现与扩展。
...
- QuickInput
  - 本地：parseNaturalInput、semantic.ts 类型定义
  - 上游：/api/v2/records、/api/v2/parse、/api/v2/user-rules
  - 下游：onRecordCreated、onAiStart、onAiDone
  - **新增**：semantic.ts、parse-rules-fallback.ts、enhance-record.ts
- RecordEditDrawer
  - 上游：/api/v2/records/{id}、/api/v2/record-links
  - 下游：onSaved、onDeleted、onError
  - **新增**：semantic.ts、enhance-record.ts
- InsightsClient
  - 上游：/api/v2/insights
...
#### 语义卡片界面
- **实现原理**：通过 chips 数组构建语义卡片，支持点击跳转到关联事项。
- **支持字段**：花费、时长、指标、时间、事项、心情、能量、状态、地点、关系人、日期等。
- **交互设计**：支持点击事项卡片跳转到对应事项，提升用户体验。

#### AI推理显示
- **实现原理**：通过 parsed_semantic.reasoning 字段显示AI的判断理由。
- **显示位置**：在记录卡片和编辑抽屉中都有相应的显示区域。
- **用户价值**：帮助用户理解AI的决策过程，提高信任度。

#### 置信度级别
- **实现原理**：通过 field_confidence 和 confidence 字段实现红绿灯机制。
- **显示方式**：低置信度时显示提醒，高风险时显示警告。
- **阈值设置**：置信度低于0.7时触发提醒，不同风险级别有不同的处理策略。

#### 建议修正功能
- **实现原理**：通过 ClarificationNeeded 类型定义和澄清框组件实现。
- **支持场景**：共享时长分配、子项归属歧义、事项归属缺失、低置信度、高风险、模糊输入等。
- **交互设计**：提供多种处理选项，包括按AI推测结果确认、修改原文、取消等。

#### 规则回退系统
- **实现原理**：当AI解析失败时，自动切换到本地规则解析模式。
- **支持场景**：API Key缺失、网络超时、API错误、网络不可用等情况。
- **用户价值**：确保系统在任何情况下都能正常工作，提供基础解析能力。

**章节来源**
- [QuickInput.tsx](file://src/app/(dashboard)/records/components/QuickInput.tsx#L1619-L1915)
- [semantic.ts:45-70](file://src/types/semantic.ts#L45-L70)
- [parse-rules-fallback.ts:284-370](file://src/lib/ai/parse-rules-fallback.ts#L284-L370)
- [enhance-record.ts:318-346](file://src/lib/ai/enhance-record.ts#L318-L346)