...
- 1.5（开发中，约60%完成）：
  - **三层九组模型**：记录结构化为 L1 原始层 → L2 主链层（9组可独立统计字段）→ L3 附属属性层
  - **语义解析引擎**：DeepSeek LLM 解析自然语言 → 结构化字段填充 → 异步 AI 增强
  - **目标引擎重构**：3 类规则（一次性完成/周期性达成/周期性限制）替代 measure_type
  - **DB 性能优化**：007 迁移新增 RLS 友好复合索引（user_id 前缀），清理旧弱索引
  - **DEV_MODE 安全加固**：服务端和客户端均实现生产环境自动禁用，防止 RLS 绕过
  - **类型去重**：Update 类型用 Omit 从 Create 类型派生，消除 ~100 行重复定义
  - **组件拆分**：QuickInput 中 ParsedChip 提取为独立组件
  - 详见 [TETO 1.5 项目说明文档](file://docs/01-生效版本/TETO 1.5/TETO_1.5_项目说明文档.md)
