...
import {
  ...
  ChevronRight, AlertTriangle, FolderInput, Activity
} from 'lucide-react';
...
import ItemFunctionTracePanel from '../components/ItemFunctionTracePanel';
import ItemKeywordRulesSection from '../components/ItemKeywordRulesSection';
import ItemKnowledgeSection from '../components/ItemKnowledgeSection';
...
export default function ItemDetailClient({ itemId }: { itemId: string }) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showError(getApiErrorMessage(errData, '更新事项失败'));
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) router.push('/items');
        ...
        const errData = await res.json();
        showError(getApiErrorMessage(errData, '搁置事项失败'));
      }
    } catch (err) {
  ...
  const handleCompleteItem = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(getApiErrorMessage(errData, '标记完成失败'));
      }
  ...
  const confirmCompleteRecord = async () => {
    ...
    try {
      ...
      if (res.ok) { setGoalRefreshKey(k => k + 1); fetchItem(); } else { const e = await res.json(); showError(getApiErrorMessage(e, '完成操作失败')); }
  ...
  const handlePostponeRecord = async (record: TetoRecord) => {
    ...
    try {
      ...
      if (res.ok) { setGoalRefreshKey(k => k + 1); fetchItem(); } else { const e = await res.json(); showError(getApiErrorMessage(e, '推迟操作失败')); }
  ...
  const handleCancelRecord = async (record: TetoRecord) => {
    ...
    try {
      ...
      if (res.ok) { setGoalRefreshKey(k => k + 1); fetchItem(); } else { const e = await res.json(); showError(getApiErrorMessage(e, '取消操作失败')); }
  ...
  const handleDeletePhase = async (phase: Phase) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json();
        showError(getApiErrorMessage(errData, '删除阶段失败'));
      }
    } catch {
      ...
      <div className="mx-auto max-w-[1200px] px-4 py-5">
          ...
          {!isCategoryView && (
            <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-slate-700">动作轨迹</h3>
              </div>
              <ItemFunctionTracePanel itemId={itemId} />
            </section>
          )}
          {!isCategoryView && (
            <ItemKeywordRulesSection
              itemId={itemId}
              defaultFunctionTagId={item.default_function_tag_id ?? null}
              defaultToolLabel={item.default_tool_label ?? null}
              onDefaultsSaved={(patch) =>
                setItem((prev) => (prev ? { ...prev, ...patch } : prev))
              }
            />
          )}
          {!isCategoryView && <ItemKnowledgeSection itemId={itemId} />}
          {isCategoryView && (
        ...
        {editingRecord && (
          <RecordEditDrawer
            ...
            onTagCreated={(tag) =>
              setRecordTags((prev) => (prev.some((t) => t.id === tag.id) ? prev : [...prev, tag]))
            }
