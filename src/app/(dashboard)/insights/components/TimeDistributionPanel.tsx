...
interface TimeDistributionPanelProps {
  data: InsightsData['time_distribution'];
}

export default function TimeDistributionPanel({ data }: TimeDistributionPanelProps) {
  if (!data) return null;
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-slate-800">时间分布</h2>
        </div>
        <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/40 p-8 text-center">
          <p className="text-sm text-slate-400">暂无时间分布数据。</p>
        </div>
      </div>
    );
  }
