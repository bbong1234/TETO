...
import { BookOpen, Clock3, Link2, Loader2, Save } from 'lucide-react';
...
import DiaryLinkedEditor, { type DiaryLinkedEditorHandle } from './DiaryLinkedEditor';
...
function formatTimeMarker(date = new Date()): string {
  return `[${pad2(date.getHours())}:${pad2(date.getMinutes())}] `;
}

export interface RecordsDiaryFooterHandle {
  getDocument: () => DiaryDocument;
  applyDocument: (doc: DiaryDocument) => void;
  insertTimeMarker: () => void;
}
...
const RecordsDiaryFooter = forwardRef<RecordsDiaryFooterHandle, RecordsDiaryFooterProps>(
  function RecordsDiaryFooter(
    {
      date,
      dayRecords = [],
      onError,
    ...
    const editorRef = useRef<DiaryLinkedEditorHandle>(null);
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        applyDocument,
        insertTimeMarker: () => {
          editorRef.current?.insertTextAtCursor(formatTimeMarker());
        },
      }),
      [applyDocument]
    );
    ...
    const toolbar = (
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          <button
            type="button"
            onClick={() => editorRef.current?.insertTextAtCursor(formatTimeMarker())}
            disabled={loading}
...
export default RecordsDiaryFooter;
